    ...
    stages {
        stage('Install & Setup Tools') {
            steps {
                ...
                rm -rf terraform* terraform-bin awscliv2.zip aws aws-cli-bin

                # --- Unzip (already present, but keep safe) ---
                if ! command -v unzip &> /dev/null; then
                    echo "unzip not found. Attempting install..."
                    if command -v apt-get &> /dev/null; then
                        apt-get update -qq && apt-get install -y unzip || echo "Warning: Could not install unzip"
                ...
                # --- AWS CLI v2 (required for aggressive bucket cleanup) ---
                ...
                if [ -s "${AWS_ZIP}" ]; then
                    ...
                    ./aws/install -i "${WORKSPACE}/aws-cli-bin" -b "${WORKSPACE}/aws-cli-bin/aws" --update || true
                    chmod +x "${WORKSPACE}/aws-cli-bin/aws"
                    echo "✅ AWS CLI v2 installed to ${WORKSPACE}/aws-cli-bin"
                else
                ...
                '''
                sh 'terraform version'
                sh 'aws --version || echo "AWS CLI ready"'
            }
        }
                ...
                script {
                    ...
                    if (destroyChoice == 'yes') {
                        withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'jenkinsTest']]) {
                            ...
                            # This permanently solves orphaned buckets from previous pipeline runs (different workspace = lost tfstate)
                            aws s3 ls | awk '{print $3}' | grep '^jenkins-bucket-andre-class7' | while read -r bucket; do
